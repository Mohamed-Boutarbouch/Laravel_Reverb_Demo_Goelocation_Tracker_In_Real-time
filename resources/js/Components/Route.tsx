...
export const Route = () => {
  const map = useMap();
  const [routeMarker, setRouteMarker] = useState<LatLngExpression | null>(null);

  const A = [34.00501829905671, -4.957129955291749] as LatLngExpression;
  const B = [34.065156561217115, -5.045428276062013] as LatLngExpression;
  ...
  useEffect(() => {
    ...
    routingControl.on("routesfound", async (e) => {
      ...
      if (routes && routes.length > 0 && routes[0].coordinates) {
        ...
        routes[0].coordinates.forEach((coord: any, index: any) => {
          setTimeout(function () {
            console.log(coord);
